...
_glmReadMTL(GLMmodel* model, char* name) {
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case '#':  			/* comment */
      /* eat up rest of line */
      throwaway = fgets(buf, sizeof(buf), file);
      break;
    case 'n':  			/* newmtl */
      throwaway = fgets(buf, sizeof(buf), file);
      nummaterials++;
  ...
  for (i = 0; i < nummaterials; i++) {
    ...
    model->materials[i].transdiff[0] = 0.0f;
    model->materials[i].transdiff[1] = 0.0f;
    model->materials[i].transdiff[2] = 0.0f;
    model->materials[i].transdiff[3] = 1.0f;
    model->materials[i].transspec[0] = 0.0f;
    model->materials[i].transspec[1] = 0.0f;
    model->materials[i].transspec[2] = 0.0f;
    model->materials[i].transspec[3] = 1.0f;
  ...
  while(fscanf(file, "%s", buf) != EOF) {
    ...
    case 'n':  			/* newmtl */
      ...
      break;
    case 'N':
      ret = fscanf(file, "%f", &model->materials[nummaterials].shininess);
      break;
    case 'T': // Tr
      switch(buf[1]) {
      case 'd':
        ret = fscanf(file, "%f %f %f",
                     &model->materials[nummaterials].transdiff[0],
                     &model->materials[nummaterials].transdiff[1],
                     &model->materials[nummaterials].transdiff[2]);
        break;
      case 's':
        ret = fscanf(file, "%f %f %f",
                     &model->materials[nummaterials].transspec[0],
                     &model->materials[nummaterials].transspec[1],
                     &model->materials[nummaterials].transspec[2]);
        break;
      case 'r':
        ret = fscanf(file, "%f", &model->materials[nummaterials].refraction);
        break;
      default:
        throwaway = fgets(buf, sizeof(buf), file);
        break;
      ...
      break;
    case 'd': // d
      ret = fscanf(file, "%f", &model->materials[nummaterials].alpha);
      break;
    case 'i': // illum
      ret = fscanf(file, "%d", &model->materials[nummaterials].shader);
      break;
    case 'r': // reflectivity
      ret = fscanf(file, "%f", &model->materials[nummaterials].reflectivity);
      break;
    case 'm':
      {
        char* map_name = 0;
        ...
        sprintf(string_literal, "%%%ds", static_cast<int>(MaxStringLength) - 1);
        //fprintf(stderr, "string_litteral = %s\n", string_litteral);

        // Check to see if we have scaled textures or not
        ret = fscanf(file, string_literal, map_name);
        if (strcmp(map_name, "-s") == 0) {
          // pick up the float scaled textures
          ret = fscanf(file, "%f %f", &scaling[0], &scaling[1]);
