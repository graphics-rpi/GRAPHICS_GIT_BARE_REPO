...
#include <Mouse.hpp>
#include <DeviceMemoryLogger.hpp>
...
SimpleScene*   GLUTDisplay::_scene                = 0;
...
void GLUTDisplay::init( int& argc, char** argv )
{
  ...
  printf("before glut init\n");
  if(!_benchmark_no_display &&_display_frames)
  {
    printf("before glut init2\n");
  glutInit(&argc, argv);
  printf("after glut init\n");
  glutInitDisplayMode(GLUT_RGB | GLUT_DOUBLE);
  printf("after glut init dispaly mode\n");
  }
  else
    printf("no glut display\n");
}


inline float3 radcoord2sketchup(const float3& v){//v is a purely radiance coordinate, not radiance
					       //rendering mode
  float3 ret;
...
void loadViewFile(std::string filename, float3& point, float3& direction, float3& up, float& horview, float& vertview )
{
  std::string sLine;
  std::ifstream oInFile(filename.c_str());

  // view position and view direction and up vector
  float3 vp, vd, vu;
  float vh, vv;
  vh = vv = M_PI/4;
  char token[256];
  ...
  while(getline(oInFile,sLine)){
    ...
    ss >> token;
    if (token=="") continue;
    bool radianceCoord = false;

    if(token[0] == '#'){
//      ignore comments
      continue;
    }
    else if(token == "rview"){// read Radiance view file
      radianceCoord = true;
    }
    else if(token == "standard"){// read standard view file, in opengl/sketchup coordinate system
      std::cerr<<"we only support Radiance view string now\n";
      continue;
    }
    else{
      std::cerr << "unknown view string, must be rview"<<std::endl;
      return;
    }
    ...
    while (ss >> token){
      if(token == "-vp"){
	ss >> token;
	vp.x = static_cast<float> (atof(token.c_str()));
	ss >> token;
	vp.y = static_cast<float> (atof(token.c_str()));
	ss >> token;
	vp.z = static_cast<float> (atof(token.c_str()));
      }
      else if(token == "-vd"){
	ss >> token;
	vd.x = static_cast<float> (atof(token.c_str()));
	ss >> token;
	vd.y = static_cast<float> (atof(token.c_str()));
	ss >> token;
	vd.z = static_cast<float> (atof(token.c_str()));
      }
      else if(token == "-vu"){
	ss >> token;
	vu.x = static_cast<float> (atof(token.c_str()));
	ss >> token;
	vu.y = static_cast<float> (atof(token.c_str()));
	ss >> token;
	vu.z = static_cast<float> (atof(token.c_str()));
      }
      else if(token == "-vh"){
	ss >> token;
	vh = static_cast<float> (atof(token.c_str()));
      }
      else if(token == "-vv"){
	ss >> token;
	vv = static_cast<float> (atof(token.c_str()));
      }
      else{
	      std::cout << "unknow view arguments: "<<token<<"\n";
      }
    }// end while (ss >> token)

    if(radianceCoord){// transfer to sketchup coord first
      vp = radcoord2sketchup(vp);
      vd = radcoord2sketchup(vd);
      vu = radcoord2sketchup(vu);
    }

//    if(oConf.eRenderingMode == eSketchupMode){// change to radiance rendering mode
//      vp = bcutil::sketchupmode2radmode(vp);
//      vd = bcutil::sketchupmode2radmode(vd);
      //vu = bcutil::sketchupmode2radmode(vu);
//    }

    // transfer the view position by the scene transformation matrix
//    if(oConf.yScaleBounds) {
//      const vec3f vp1 = vp;
//      mat43TransformPoint3(&vp, oFrame.getSceneTransform(), &vp1);
//    }
    // view direction
//    vec3SafeNormalize(&vd);

//    views.push_back(CView_Radiance());
    point = vp;
    direction = vd;
    up=vu;
    horview = vh*M_PI/180;
    vertview = vv*M_PI/180;
...
void GLUTDisplay::run( const std::string& title, SimpleScene* scene, contDraw_E continuous_mode )
{
  ...
  if(_display_frames)
  {
    ...
    printf("after glut init again\n");
    glutInitWindowPosition(100,100);
    printf("after init window position\n");
    glutCreateWindow( _title.c_str() );
    printf("after glut create windows\n");
    glutHideWindow();
  #if !defined(__APPLE__)
    glewInit();
  ...
  try {
    ...
    if ( !_camera_pose.empty() )
      camera_data = SimpleScene::InitialCameraData( _camera_pose );

    // Initialize camera according to scene params
//    _camera = new PinholeCamera( point,//camera_data.eye,
//                                 dir, //camera_data.lookat,
//                                 up,//camera_data.up,
...
void GLUTDisplay::setCamera(SimpleScene::InitialCameraData& camera_data)
{
...
{
  bool will_display=_display_frames;//_scene->activeDisplay();
  ...
  try {
    ...
    if( will_display&&_display_frames) {
    ...
    printf("Leaving Catch and Program?\n");
    sutilReportError( e.getErrorString().c_str() );
    exit(2);
  }
...
void GLUTDisplay::quit(int return_code)
{
  try {
    if(_scene)
    {
      ...
      {
        sutilReportError( "Derived scene class failed to call SimpleScene::cleanUp()" );
        exit(2);
      }
    }
    exit(return_code);
...
{
  // Set up scene
  printf("begin benchmark no display\n");
  SimpleScene::InitialCameraData initial_camera_data;
  printf("bnp0\n");
