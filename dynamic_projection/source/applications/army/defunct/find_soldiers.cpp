...
public:
  ...
  Ramp(Image<byte> &component_image,
    ...
    int most_inliers = 0;
    for(int i = 0; i < 4; i++){
      if(edge_inliers[i].size() > most_inliers){
	ground_edge_id = i;
...
public:
  Soldier(Image<byte> &component_image, std::vector<point> &points,
    ...
    // discard if the object is too "scattered"
    if(edge_points.size() >= 0.4 * points.size()){
      valid = false;
      return;
    }
  ...
  void draw(Image<sRGB> &image){
    ...
    for(int i = -5; i < 6; i++){
      image(center_row + i, center_col + i) = sRGB(255, 255, 255);
      image(center_row + i, center_col - i) = sRGB(255, 255, 255);
      image(center_row - i, center_col + i) = sRGB(255, 255, 255);
      image(center_row - i, center_col - i) = sRGB(255, 255, 255);
    }
...
#warning hack: align coordinate system with camera frame (correct solution)
    ...
    FILE *fp = fopen(temp_filename.c_str(), "wt");
    if (NULL == fp){
...
#define TABLE
    ...
    */
    ...
    rename(temp_filename.c_str(), filename);
  }
...
public:
  ...
  void classify(){
    ...
    if(color == RED_IDX || color == GREEN_IDX){
      // army man!
...
#define DEBUG_LABELS_IMAGE
  ...
  if (WRITE_DEBUG_IMAGES){
    label_debug.write("debug_labels/debug_labels.pgm");
