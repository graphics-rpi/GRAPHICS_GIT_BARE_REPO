...
public:
  ...
  Image<point> getPointImage(){
    Image<point> image(point_rows-1, point_cols-1);//Josh: why is this -1 ?
    for (int row=0; row<image.getRows(); row++){
      for (int col=0; col<image.getCols(); col++){
...
public:
  ...
  void findMask(Image<byte> &wall_mask){
...
//	      int range_thresh = 20;+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        // for some values and this part of the mask should be activated
	      if (hist.range() > range_thresh && wall_mask(row,col)>127){
    ...
    // This has to be done here because the mask changed in KeepLargestComponent
    int pixel_count = 0;
  ...
  // New mask is the mask inserted to crop for EMPAC screens
  void findIds(/*const Image<byte> &new_mask */){
  //adjusted parameters
//HACK!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
      min_range*=.7;
    ...
    for (int row=0; row<rows; row++){
      for (int col=0; col<cols; col++){
        ...
        if (/*new_mask(row,col)<127||*/!mask_image(row, col) || hist.range() < min_range){
          id_image(row, col) = PointIdx(-1, -1);
        } else {
          byte thresh = hist.median();  // Is this average pixel intensity ??????
          int row_id = 0;
	  int i;
    ...
    printf("entering find mask 5\n");
    // keep only the largest contiguous block of each col index
...
void WritePointPairs(const char *filename, Image<point> &OutputPoints,
  ...
  for (int row=0; row<OutputPoints.getRows(); row++){
    ...
    if (InputPoints(row,col).row == -1 &&
    	  InputPoints(row,col).col == -1)
        printf("neg1\n");
    else  if (fabs(InputPoints(row,col).row) > 0.5 &&
...
int set_projector_level (CameraAPI<sRGB> *camera, int rows, int cols,
			 SDL_Surface *screen, const Image<byte> &mask){
  int level = 255;
  int step = 128;
  ...
  do {
	...
	if (255 == in_image(row, col)&&mask(row,col)>127){
...
//  int point_cols = 12;
 ...
 int image_cols = 1280;
  ...
  //  Image<byte> EMPAC_mask(mask_filename);
  Image<byte> EMPAC_blackandwhite_mask(mask_filename);

  //Image<byte> EMPAC_blackandwhite_mask=rgb_to_gray(EMPAC_mask);
  int level;
  char level_string[256];
  sprintf(level_string,"%s_%s_level.txt",argv[5],argv[6]);  
  if(load)
  {
    std::ifstream input_level(level_string);
    input_level>>level;
  }
  else
  {
    level = set_projector_level (&camera, rows, cols, screen,EMPAC_blackandwhite_mask);
    std::ofstream output_level(level_string);
    output_level<<level;
  }
...
//  double a0 = cal_cam.getA0();
 ...
 char test_filename[256];
  ...
  for (int i=0; i<n_masks; i++){
    ...
    sprintf(test_filename,"calibration_images_1/%s_%s_%i.pgm",argv[5],argv[6],i);
    Image<byte> in_image;
    if(load)
    {
      in_image = Image<byte>(test_filename);
    }
    else
    {   
    draw_8bit_image(screen, mask_image, level); 
    in_image = get_image(&camera);


    in_image.write(test_filename);
    //Undistorts image (not currently used)
    //Image<byte> undistorted = CorrectRadialDistortion(in_image, 
		//				      u0, v0, k1, k2, a0);
    //calrec.addImage(undistorted);
    }
    calrec.addImage(in_image);
  }

  calrec.findMask(EMPAC_blackandwhite_mask);
  calrec.clearImages();
 
  for (int i=0; i<n; i++){
    sprintf(test_filename,"calibration_images_2/%s_%s_%i.pgm",argv[5],argv[6],i);
    Image<byte> in_image;
    if(load)
    {
      in_image = Image<byte>(test_filename);
  ...
  calrec.findIds(/*EMPAC_blackandwhite_mask*/);
  ...
  //Co-ordinates of the grid in projector space
  Image<point> lcd_points = calgen.getPointImage();

  //Co-ordinates of the grid in camera space
  //Floating point X-Y locations in camera space stored in smaller (point rows by point cols) image
