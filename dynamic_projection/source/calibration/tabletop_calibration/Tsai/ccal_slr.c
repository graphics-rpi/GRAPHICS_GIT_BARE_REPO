    ...
    char    **argv;
{
    ...
    if (argc != 8) {
	fprintf (stderr, "syntax: %s <input file> <width> <height>"
		 " <output file> <xoff> <yoff> <zoff>\n", argv[0]);
	exit (-1);
    }

    /* initialize the camera parameters (cp) with the
       appropriate camera constants */
    initialize_EOS_50D_parms (); // tcy 9/2/08
