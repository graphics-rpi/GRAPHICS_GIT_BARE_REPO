...
/*export*/ template <class T>
MRMatrix<T>::MRMatrix(const MRMatrix<T> &other) {
...
/*export*/ template <class T>
MRMatrix<T>::MRMatrix(int rows, int columns) {
	m_matrix = NULL;
	resize(rows, columns);
}
...
/*export*/ template <class T>
MRMatrix<T> &
MRMatrix<T>::operator= (const MRMatrix<T> &other) {
	if ( other.m_matrix != NULL ) {
		// copy arrays
		resize(other.m_rows, other.m_columns);
		for ( int i = 0 ; i < m_rows ; i++ )
...
/*export*/ template <class T>
void
MRMatrix<T>::resize(int rows, int columns) {
	if ( m_matrix == NULL ) {
		// alloc arrays
...
/*export*/ template <class T>
MRMatrix<T> 
MRMatrix<T>::product(MRMatrix<T> &other) {
	assert( m_matrix != NULL );
	assert( other.m_matrix != NULL );
	assert ( m_columns == other.m_rows );

	MRMatrix<T> out(m_rows, other.m_columns);
...
/*export*/ template <class T>
T&
MRMatrix<T>::operator ()(int x, int y) {
	assert ( x >= 0 );
	assert ( y >= 0 );
	assert ( x < m_rows );
	assert ( y < m_columns );
