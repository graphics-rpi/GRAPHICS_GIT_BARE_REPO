...
public:
  ...
  */
  ...
  double Length() const {
    double l = (double)sqrt( data[0] * data[0] + data[1] * data[1] );
    return l; }
...
public:
  ...
  */
  ...
  double Length() const {
    double l = (double)sqrt( data[0] * data[0] +
			     data[1] * data[1] +
			     data[2] * data[2] );
    return l; }
  bool isNaN() const {
    return (my_isnan(data[0]) || my_isnan(data[1]) || my_isnan(data[2]));
    //return (std::isnan(data[0]) || std::isnan(data[1]) || std::isnan(data[2]));
  }

  // MODIFIERS
  void zero_y() { data[1] = 0; }
  ...
  */
  ...
  static void interpolate(const Vec3f &a, const Vec3f &b, double s, Vec3f &answer) {
    answer = b;
    answer -= a;
    answer *= s;
    answer += a; }
...
inline std::ostream &operator<<(std::ostream &outstream, const Vec3f &v) {
  outstream << "Vec3f <" <<v.x()<<", "<<v.y()<<", "<<v.z()<<">"; //<<std::endl;
  return outstream;
}

inline void OUTPUT(std::ostream &outstream, const Vec3f &v, const std::string &s) {
  outstream << s.c_str() << " " << v.x() << " " << v.y() << " " << v.z() << std::endl;
}
